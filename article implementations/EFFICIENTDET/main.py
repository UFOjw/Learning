...
def get_args():
    ...
    parser.add_argument("--input", type=str, default="test_videos\\14.mp4")
...
def test(opt):
    ...
    while cap.isOpened():
        ...
        for box_id in range(boxes.shape[0]):
            ...
            Point1 = torch.floor(xmin).long().item(), torch.floor(ymin).long().item()
            Point2 = torch.floor(xmax).long().item(), torch.floor(ymax).long().item()
            color = colors[pred_label]
            cv2.rectangle(output_image, Point1, Point2, color, 2)
            text_size = cv2.getTextSize(COCO_CLASSES[pred_label] + ' : %.2f' % pred_prob, cv2.FONT_HERSHEY_PLAIN, 1, 1)[0]
            cv2.rectangle(output_image, Point1, (Point1[0] + text_size[0] + 3, Point1[1] + text_size[1] + 4), color, -1)
            cv2.putText(
                output_image, COCO_CLASSES[pred_label] + ' : %.2f' % pred_prob,
                (Point1[0], Point1[1] + text_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 1,
                (255, 255, 255), 1)
